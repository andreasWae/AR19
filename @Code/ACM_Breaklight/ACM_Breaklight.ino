...
void loop() {
  if (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK) {
    ...
    breakpressurepercent = ((breakPressure1+breakPressure2)/2)*(100/255); //Gjennomsnittet av bremsetrykket fra 0 til 100% er det bedre å bruke map()?
    ...
    if (breakpressurepercent >= pressureThreshold) {
      digitalWrite(rled, LOW); //Debug led blir rødt
